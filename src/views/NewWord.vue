  ...
  <div class="home-view">
    <nav>
      ...
      <div class="logo">
        ...
        <div class="subtitle">
          ...
          Aggiungi una nuova parola personale
        </div>
    </div>
    ...
    <main class="words single">
      ...
      <div class="quotes" style="flex-flow: column;">
        <p>Modifica il testo che trovi a sinistra, poi clicca su <i>Aggiungi Parola</i> qui sotto:</p>
        <button v-on:click="addWord()">Aggiungi Parola</button>
      </div>
    </main>

    <p class="error" style="color: red" v-if="error[0].show">{{error[0].text}}</p>
...
export default {
  ...
  methods: {
    ...
    addWord: function() { // funzione per aggiungere la parola allo spazio personale
      ...
      if(userWords.includes(this.newWord)) {
        this.error[0].text = "La parola che hai provato ad inserire esiste già.";
        this.error[0].show = true;
        console.log('duplic');
